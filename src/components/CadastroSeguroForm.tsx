...
const schema = z.object({
  ...
  cpf: z.string().min(1, { message: 'Campo obrigatório' }),
  endereco: z.string().min(1, { message: 'Campo obrigatório' }),
  inicioVigencia: z.string().refine(value => /^\d{2}\/\d{4}$/.test(value), {
    message: 'Formato inválido. Use DD/MM/AAAA',
  }),
  fimVigencia: z.string().refine(value => /^\d{2}\/\d{4}$/.test(value), {
    message: 'Formato inválido. Use DD/MM/AAAA',
  }),
});

type CadastroSeguroFormInputs = z.infer<typeof schema>;
...
const CadastroSeguroForm: React.FC = () => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-md mx-auto mt-8">
      <div className='grid grid-cols-2 gap-4 sm:grid-cols-2 lg:grid-cols-3'>
        {/* Campos do formulário (omiti para brevidade) */}
      </div>
      {/* Botão de Envio */}
      <div className="flex items-center justify-between">
        <button
